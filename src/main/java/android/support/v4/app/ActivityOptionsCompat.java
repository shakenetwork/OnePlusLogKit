...
public class ActivityOptionsCompat {
    ...
    public static ActivityOptionsCompat makeSceneTransitionAnimation(Activity activity, Pair<View, String>... sharedElements) {
        ...
        if (sharedElements != null) {
            viewArr = new View[sharedElements.length];
            strArr = new String[sharedElements.length];
            for (int i = 0; i < sharedElements.length; i++) {
                viewArr[i] = (View) sharedElements[i].first;
                strArr[i] = (String) sharedElements[i].second;
            }
        }
        if (VERSION.SDK_INT >= 24) {
